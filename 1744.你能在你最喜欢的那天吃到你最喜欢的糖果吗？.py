...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        presums = list(accumulate(candiesCount,initial=0))
        ret = [False] * len(queries)
        for i, (type, day, cap) in enumerate(queries):
            earliest=presums[type]//cap+1
            latest=presums[type+1]
            if earliest<=day+1<=latest:
                ret[i]=True
        return ret
...
) == eval_list_str("[false,true,true,false,false]")
assert Solution().canEat(
    candiesCount=[7, 4, 5, 3, 8], queries=[[0, 2, 2], [4, 2, 4], [2, 13, 1000000000]]
) == eval_list_str("[true,false,true]")
